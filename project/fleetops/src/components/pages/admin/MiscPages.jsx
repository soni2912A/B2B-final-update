...
export function OccasionsPage() {
  ...
  const [occAddedCount, setOccAddedCount] = useState(0)
  ...
  const [viewTab, setViewTab]           = useState('all')   // 'all' | 'manual' | 'google'
  ...
  async function deleteOccasion(id) {
    ...
    setDeletingId(id)
    try {
      await apiFetch('DELETE', `/admin/occasions/${id}`)
  ...
  const filteredOccasions = occasions.filter(o => {
    if (viewTab === 'manual') return o.source !== 'google'
    if (viewTab === 'google') return o.source === 'google'
    return true
  })

  const filteredEventMap = {}
  for (const occ of filteredOccasions) {
    const d = new Date(occ.date)
    if (d.getFullYear() === year && d.getMonth() === month) {
      const day = d.getDate()
      ;(filteredEventMap[day] = filteredEventMap[day] || []).push(occ)
    }
  }
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {[['all','All Events'],['manual','Manual'],['google','Google Calendar']].map(([val, label]) => (
          <button key={val} type="button" onClick={() => setViewTab(val)}
            className={`px-3 py-1.5 rounded-full text-[12px] font-medium border transition-colors ${
              viewTab === val ? 'bg-accent text-white border-accent' : 'bg-surface border-border2 text-text2 hover:bg-surface2'
            }`}>{label}</button>
        ))}
        <span className="flex items-center gap-2 ml-2 text-[11px] text-text2">
          <span className="w-2 h-2 rounded-full bg-green-400 inline-block"/>Birthday
          <span className="w-2 h-2 rounded-full bg-blue-400 inline-block"/>Anniversary
          <span className="w-2 h-2 rounded-full bg-orange-400 inline-block"/>Holiday
          <span className="w-2 h-2 rounded-full bg-gray-400 inline-block"/>Other
          <span className="w-2 h-2 rounded-full bg-purple-400 inline-block"/>Google
        </span>
      </div>
          ...
          >
            ...
            <div className="grid grid-cols-7 gap-1 min-w-[280px]">
              ...
              {Array.from({ length: daysInMonth }, (_, i) => {
                const day = i + 1
                const evs = filteredEventMap[day] || []
                ...
                return (
                  <button
                    ...
                    type="button"
                    role="gridcell"
                    onClick={() => openAddForDay(isoYMD(date))}
                    title={tooltip}
                    aria-label={tooltip}
      ...
      {showAdd && (
        <Modal
          title={occAddedCount > 0 ? `Add Occasion (${occAddedCount} added)` : "Add Occasion"}
          onClose={() => { setShowAdd(false); setOccForm(EMPTY_OCC_FORM); setOccFormError(''); setOccAddedCount(0) }}
          actions={[
            {
              label: occSaving ? 'Adding…' : 'Add Occasion',
              primary: true,
              disabled: occSaving,
              onClick: async () => {
...
export function InventoryPage() {
  ...
  const [initialLoad, setInitialLoad] = useState(true)  // FIX: separate initial vs refresh loading
  ...
  async function load(isRefresh = false) {
    ...
    try {
      ...
      console.log('[Inventory] raw API response:', JSON.stringify(res))
      const fetched = res.data?.items ?? res.items ?? res.data ?? []
      console.log('[Inventory] items to set:', fetched?.length, fetched)
  ...
  return (
    <div>
      <PageHeader title="Inventory Management" subtitle="Track stock levels and warehouse movement">
        {refreshing && <span className="text-[12px] text-text2 animate-pulse mr-2">Refreshing…</span>}
      ...
      <Card noPad>
        <TableWrap>
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-10 text-text2 text-sm">No inventory items found.</td></tr>
            ) : items.map(item => {
              const maxBar = Math.max((item.alertThreshold || 20) * 3, item.stock, 1)
              const barPct = Math.round((item.stock / maxBar) * 100)
              return (
                <tr key={item._id}>
                  ...
                  <td>
                    <div className="flex items-center gap-2">
                      ...
                      <div className="w-14 h-1.5 bg-surface2 rounded overflow-hidden">
                        <div className={`h-full rounded ${item.stock === 0 ? 'bg-red-400' : item.stock <= (item.alertThreshold || 20) ? 'bg-amber-400' : 'bg-accent'}`} style={{ width: `${barPct}%` }} />
                      </div>
                    </div>
                  </td>
...
function AdjustStockModal({ items, onClose, onSave }) {
  const [form, setForm]   = useState({ productId: items[0]?._id || '', type: 'in', quantity: '', reason: '' })
  const [saving, setSaving] = useState(false)
  ...
  async function save() {
    ...
    if (!Number.isInteger(qty))        { setError('Quantity must be a whole number.'); return }
    ...
    try {
      ...
      await onSave()  // FIX: await onSave so modal stays open until data is refreshed
    } catch (err) {
      setError(err.message || 'Adjust failed. Please try again.')
      setSaving(false)
    }
  }
  ...
  return (
    <Modal title="Adjust Stock" onClose={onClose}
      actions={[
        { label: saving ? 'Saving…' : 'Adjust', primary: true, onClick: save, disabled: saving },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <FormGroup label="Adjustment Type">
          <Select value={form.type} onChange={set('type')}>
            ...
            <option value="adjustment">Manual Adjustment (signed delta)</option>
          </Select>
        </FormGroup>
        {/* FIX: placeholder & min=1 so user knows 0 is invalid */}
        <FormGroup label="Quantity">
          <Input type="number" min="1" step="1" placeholder="e.g. 50" value={form.quantity} onChange={set('quantity')} />
        </FormGroup>
      </div>
      <FormGroup label="Reason (optional)">
        <Textarea value={form.reason} onChange={set('reason')} placeholder="e.g. Purchase, Return, Damaged, Audit Correction" />
      </FormGroup>
    </Modal>
  )
}
