    ...
    <div>
      ...
      {showInvite && (
        ...
        >
          ...
          <FormGroup label="Name">
            <Input placeholder="Full name" value={inviteForm.name} onChange={e => { if (/^[^0-9]*$/.test(e.target.value)) setInviteForm(f => ({ ...f, name: e.target.value })) }} />
          </FormGroup>
      ...
      {editing && (
        ...
        >
          ...
          <FormGroup label="Name">
            <Input value={editForm.name} onChange={e => { if (/^[^0-9]*$/.test(e.target.value)) setEditForm(f => ({ ...f, name: e.target.value })) }} />
          </FormGroup>
          <FormGroup label="Role">
...
export function SettingsPage() {
  ...
  async function saveInfo() {
    ...
    if (info.phone.trim() && info.phone.replace(/\D/g,'').length !== 10) { setInfoError('Phone must be exactly 10 digits.'); return }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <Card>
          ...
          <FormGroup label="Phone">
            <Input value={info.phone} onChange={e => { const v = e.target.value.replace(/[^0-9]/g,'').slice(0,10); setInfo(i => ({ ...i, phone: v })) }} />
          </FormGroup>
          ...
          <div className="mb-3 p-2.5 rounded border border-green-200 bg-green-50 text-[12px]">
            ...
            {status.connectedAt && (
              <div className="text-text2 mt-0.5 pl-5">
                Connected on {new Date(status.connectedAt).toLocaleDateString('en-GB', { day:'2-digit', month:'2-digit', year:'numeric' })}
              </div>
            )}
          </div>
