...
export function SASubscriptionsPage() {
  ...
  async function deletePlan(plan) {
    if (!window.confirm(`Are you sure you want to delete the "${plan.name}" plan? This cannot be undone.`)) return
    setDeleting(plan._id)
    try {
      await apiFetch('DELETE', `/super-admin/subscriptions/${plan._id}`)
  ...
  function planEndDate(plan) {
    const start = plan.createdAt ? new Date(plan.createdAt) : null
            ...
            return (
              <div key={p._id} className={`bg-surface border rounded-lg overflow-hidden ${popular ? 'border-accent' : 'border-border'}`}>
                ...
                <div className="p-6">
                  ...
                  {}
                  {p.createdAt && (
                    <div className="mb-4 px-3 py-2 rounded-md bg-surface2 text-[11px] text-text2 border border-border">
                      <div className="flex justify-between mb-0.5">
                        <span>Start Date</span>
                        <span className="font-medium text-text1">{formatDate(p.createdAt)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Next Renewal</span>
                        <span className="font-medium text-text1">{endDate ? formatDate(endDate) : '—'}</span>
                      </div>
                    </div>
                  )}
