...
const getAllLoginLogs = async (req, res) => {
  try {
    ...
    return handleError(res, error, 'getAllLoginLogs');
  }
};
...
const exportLoginLogs = async (req, res) => {
  try {
    ...
    const handled = importExportService.sendExport(res, format, dataset, `login-logs-${Date.now()}`);
    if (!handled) return sendError(res, 400, 'Invalid format.');
  } catch (error) {
    return handleError(res, error, 'exportLoginLogs');
  }
};
